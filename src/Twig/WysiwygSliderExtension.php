...
class WysiwygSliderExtension extends AbstractExtension
{
    ...
    public function parseWysiwygSlider($value)
    {
        ...
        if (!empty($matches[0])) {
            ...
            $middleSlider = substr($debSlider, 0, $matchesEnd[0][(sizeof($matchesEndItem[0]) * 3) + 1][1] + strlen("</div>"));

            $endSlider = substr($debSlider, $matchesEnd[0][(sizeof($matchesEndItem[0]) * 3) + 1][1] + strlen("</div>"));
